...
export const ThemeProvider: FunctionComponent<
  PropsWithChildren<{ value: ThemeContentInitialData }>
> = ({ value: { theme }, children }) => {
  const [themeState, setThemeState] = useState<ThemeContextData["theme"]>(
    theme
  ) as [ThemeContextData["theme"], ThemeContextData["setTheme"]];
  ...
  useLayoutEffect(() => {
    ...
    if (
      ...
      ["light", "dark", "system"].includes(storedState)
    )
...
export const Body: FunctionComponent<JSX.IntrinsicElements["body"]> = ({
  className,
  children,
  ...props
}) => {
  ...
  return (
    <body className={`${themeClass} ${className}`.trim()} {...props}>
      {children}
    </body>
  );
};
